...
function fakeBackendFactory(backend, options, realBackend) {
    ...
    var invoiceOwner = JSON.parse(localStorage.getItem('invoiceOwner')) || [];
    ...
    backend.connections.subscribe(function (connection) {
        ...
        setTimeout(function () {
            ...
            if (connection.request.url.endsWith('http://localhost:1006/jsflab/rest/OwnerRegistration/ownerregistration') && connection.request.method === http_1.RequestMethod.Post) {
                // get new user object from post body
                var newUser_1 = JSON.parse(connection.request.getBody());
                // validation
                var duplicateUser = invoiceOwner.filter(function (invoiceuser) { return invoiceuser.userName === newUser_1.username; }).length;
                if (duplicateUser) {
                ...
                localStorage.setItem('invoiceOwner', JSON.stringify(users));
                // respond 200 OK
            ...
            if (connection.request.url.endsWith('/api/users') && connection.request.method === http_1.RequestMethod.Post) {
                // get new user object from post body
                var newUser_2 = JSON.parse(connection.request.getBody());
                // validation
                var duplicateUser = invoiceOwner.filter(function (invoiceuser) { return invoiceuser.userName === newUser_2.username; }).length;
                if (duplicateUser) {
                    return connection.mockError(new Error('Username "' + newUser_2.username + '" is already taken'));
                }
                // save new user
                newUser_2.id = users.length + 1;
                users.push(newUser_2);
                localStorage.setItem('invoiceOwner', JSON.stringify(users));
                // respond 200 OK
                connection.mockRespond(new http_1.Response(new http_1.ResponseOptions({ status: 200 })));
                return;
